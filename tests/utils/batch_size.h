 ...
 * TODO find Asgard's formula for batch count, current one can generates batches too large to be allocated
 * without the min
 */
int compute_batch_size(int const degree, int const dimension, int const grid_level,
                       int const nb_distinct_outputs)
{
    // Kronmult parameters
    ...
    long long const max_batch_count =
        (max_element_number - nb_distinct_outputs * size_input) /
        static_cast<long long>(size_input * (2 + matrix_count * matrix_size * matrix_size));
    // formula with theorical batch count
    long long const formula_batch_count =
        pow_int(2, grid_level) * pow_int(grid_level, std::min(1, dimension - 1));
